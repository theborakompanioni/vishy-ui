...
(function (VisSense, factory) {
  ...
  VisSense.Client = VisSense.Client || {};
  VisSense.Client.createSimpleClientMonitor = factory(VisSense, VisSense.Utils);
})(VisSense, function (VisSense, Utils) {
  'use strict';
  ...
  var send = function (client, eventCollection, event) {
    ...
    client.addEvent(eventCollection, data, function (err) {
      if (err) {
        // there was an error!
        console.log('error on event ' + eventCollection);
      }
  ...
  return function (visobj, client, config) {

    var _config = Utils.extend(Utils.defaults(config, {}), {
      monitorId: uuid()
    });

    var monitorConfig = Utils.defaults(_config, {
      start: function () {
        console.log('[simple-client-monitor] start');
      },
      stop: function () {
        console.log('[simple-client-monitor] stop');
      },
      visible: function () {
        console.log('[simple-client-monitor] visible');
      },
      hidden: function () {
        console.log('[simple-client-monitor] hidden');
      },
      fullyvisible: function () {
        console.log('[simple-client-monitor] fully visible');
      },
      update: function () {
        console.log('[simple-client-monitor] update');
      },
      strategy: [
        new VisSense.VisMon.Strategy.PollingStrategy({interval: 1000}),
        new VisSense.VisMon.Strategy.EventStrategy({debounce: 30})
    ...
    if (!Utils.isArray(monitorConfig.strategy)) {
      monitorConfig.strategy = [monitorConfig.strategy];
    }

    monitorConfig.strategy = monitorConfig.strategy.concat([
      new VisSense.VisMon.Strategy.MetricsStrategy(),
      sendPercentageTimeTestEventStrategy(client, _config),
      sendInitialRequestEventStrategy(client, _config),
      sendTimeReportEventStrategy(client, _config)
    ]);
